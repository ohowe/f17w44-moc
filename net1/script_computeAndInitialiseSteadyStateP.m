...
lgg(1)=lossCoeff(4,2)*0.2;  %loss coefficient at fixture 4
lossCoeff(5,1) = lossCoeff(3,2) + lossCoeff(5,1);
lossCoeff(5,2) = lossCoeff(5,1) + lossCoeff(5,2);
lgg(2)=lossCoeff(5,2)*0.2;%loss coefficient at fixture 5
...
lgg(3)=lossCoeff(8,2)*0.002; %loss coefficient at fixture8 
...
for i=1:3
    if ss(i)>0.5 %(ss(i)=0 then fixture starts clkosed if ss(i)=1 then open) 
        Qss(i)=sqrt(2 * g * (H0SS - 24.9) / (500*lgg(i)));
    else
        Qss(i)=0;
    end
end
...
pipe(1,1).Ho = H0SS - [0:pipe(1,1).Nx/2]' * 2 * pipe(1,1).R * (Qss(1)+Qss(2)) ^ 2;%head throughout first pipe at steady state 
